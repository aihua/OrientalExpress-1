...
public class GroupConverter extends AbstractFieldInstructionConverter {
	...
	public GroupValue convert(Field field, ConversionContext context) {
		...
		FastMessage groupMsg = convert(group, new FastMessage(SessionControlProtocol_1_1.GROUP_INSTR), context);
	...
	public static FastMessage convert(Group group, FastMessage groupMsg, ConversionContext context) {
		...
		if (group.getTypeReference() != null && !FastConstants.ANY_TYPE.equals(group.getTypeReference())) {
			GroupValue typeRef = new GroupValue((Group) SessionControlProtocol_1_1.TYPE_REF.getField(new QName("TypeRef",
					SessionControlProtocol_1_1.NAMESPACE)));
			setName(typeRef, group.getTypeReference());
			groupMsg.setFieldValue("TypeRef", typeRef);
		}
		SequenceValue instructions = new SequenceValue(
