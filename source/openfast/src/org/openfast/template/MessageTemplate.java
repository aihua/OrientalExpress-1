...
public class MessageTemplate extends Group implements FieldSet {
	 ...
	 */
	public byte[] encode(FastMessage message, Context context) {
		if (!context.getTemplateRegistry().isRegistered(message.getTemplate()))
			throw new FastException("Cannot encode message: The template " + message.getTemplate()
					+ " has not been registered.", FastConstants.D9_TEMPLATE_NOT_REGISTERED);
	 ...
	 */
	public FastMessage decode(InputStream in, int templateId, BitVectorReader presenceMapReader, Context context) {
		try {
			...
			FastMessage message = new FastMessage(this, fieldValues);
			if (context.isTraceEnabled())
