...
public class XMLMessageTemplateLoader implements MessageTemplateLoader {
	...
	static final ErrorCode IO_ERROR = new ErrorCode(FastConstants.STATIC, -1, "IOERROR", "IO Error",
			FastAlertSeverity.ERROR);

	static final ErrorCode XML_PARSING_ERROR = new ErrorCode(FastConstants.STATIC, -1, "XMLPARSEERR",
			"XML Parsing Error", FastAlertSeverity.ERROR);
	...
	private Document parseXml(InputStream templateStream) {
		...
		try {
			...
			initialContext.getErrorHandler().error(XML_PARSING_ERROR,
					"Error occurred while parsing xml template: " + e.getMessage(), e);
		}
