...
public class Util {
	...
	public static ScalarValue getDifference(byte[] newValue, byte[] priorValue) {
		...
		if (prependLength < appendLength) {
			return new TwinValue(new IntegerValue(prependIndex - priorValue.length - 2), new ByteVectorValue(newValue, 0,
					prependLength));
		}
		return new TwinValue(new IntegerValue(priorValue.length - appendIndex), new ByteVectorValue(newValue, appendIndex,
				appendLength));
	}
	...
	public static int timeToInt(Date date) {
		...
		cal.setTimeZone(TimeZone.getTimeZone("GMT"));
		cal.setTime(date);
		return cal.get(Calendar.HOUR_OF_DAY) * 10000000 + cal.get(Calendar.MINUTE) * 100000 + cal.get(Calendar.SECOND)
	...
	public static ComposedScalar composedDecimal(QName name, Operator exponentOp, ScalarValue exponentVal,
			Operator mantissaOp, ScalarValue mantissaVal, boolean optional) {
