...
import sse.ngts.common.plugin.fieldtype.BooleanField;
...
import sse.ngts.common.plugin.step.field.converter.DecimalConverter;
...
public abstract class FieldMap implements Serializable {
	...
	public void setBytes(int field, byte[] value) {
		...
		final String s = ByteConverter.convert(value);
	...
	public void setLong(int field, long value) {
		final String s = LongConverter.convert(value);
		setField(new StringField(field, s));
	}
	...
	public long getLong(int field) throws FieldNotFound {
		final String value = getField(field).getValue();
		try {
	...
	public void setField(LongField field) {
		setLong(field.getField(), field.getValue());
	}
...
//			throw new FieldException(SessionRejectReason.INCORRECT_DATA_FORMAT_FOR_VALUE, field.getField());
//		} else {
//			return (BytesField) returnField;
//		}
		try {
			final String value = getField(field.getField()).getValue();
			field.setObject(ByteConverter.convert(value));
		} catch (final FieldConvertError e) {
	...
	public LongField getField(LongField field) throws FieldNotFound {
		try {
			final String value = getField(field.getField()).getValue();
			field.setObject(LongConverter.convert(value));
		} catch (final FieldConvertError e) {
			throw newIncorrectDataException(e, field.getField());
		} catch (final FieldNotFound e) {
			throw e;
	...
	protected void initializeFrom(FieldMap source) {
		...
		final Iterator<Map.Entry<Integer, List<Group>>> groupItr = source.groups.entrySet().iterator();
		while (groupItr.hasNext()) {
			...
			for (int i = 0; i < groupMembers.size(); i++) {
				...
				final Group clonedGroup = new Group(originalGroup.getFieldTag(), originalGroup.delim(),
	...
	protected void calculateString(StringBuffer buffer, int[] preFields, int[] postFields) {
		...
		for (final Field<?> field2 : fields.values()) {
			...
			if (!isOrderedField(tag, preFields) && !isOrderedField(tag, postFields) && !isGroupField(tag)) {
				field.toString(buffer);
				buffer.append('\001');
