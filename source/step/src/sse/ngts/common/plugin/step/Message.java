...
import sse.ngts.common.plugin.step.field.Signature;
import sse.ngts.common.plugin.step.field.SignatureLength;

//import sse.ngts.eztrader.step.field.MessageEncoding;

/**
 * Represents a STEP message.
 */
public class Message extends FieldMap {
	...
	private void toXMLFields(Element message, String section, FieldMap fieldMap, DataDictionary dataDictionary)
		...
		while (fieldItr.hasNext()) {
			...
			if (dataDictionary != null) {
				...
				final String enumValue = dataDictionary.getValueName(field.getTag(), field.getObject().toString());
				if (enumValue != null) {
	...
	public static class Header extends FieldMap {
		static final long serialVersionUID = -3193357271891865972L;
		private static final int[] EXCLUDED_HEADER_FIELDS = { BodyLength.FIELD, MsgType.FIELD };
	...
	public void fromString(String messageData, DataDictionary applicationDictionary, boolean doValidation)
			throws InvalidMessage {

		parse(messageData, applicationDictionary, doValidation);
	}

	void parse(String messageData, DataDictionary dataDictionary, boolean doValidation) throws InvalidMessage {
		this.messageData = messageData;
		try {
			parseHeader(dataDictionary, doValidation);
			parseBody(dataDictionary, doValidation);
			if (doValidation) {
	...
	private void parseHeader(DataDictionary dd, boolean doValidation) throws InvalidMessage {
		final boolean validHeaderFieldOrder = isNextField(dd, header, BeginString.FIELD)
				&& isNextField(dd, header, BodyLength.FIELD) && isNextField(dd, header, MsgType.FIELD);
			...
			if (field.getTag() == firstField) {
				...
				if (groupDataDictionary.isGroup(msgType, field.getField())) {
					...
					if (groupDataDictionary.isField(field.getTag())) {
						if (!firstFieldFound) {
							throw new FieldException(SessionRejectReason.REPEATING_GROUP_FIELDS_OUT_OF_ORDER, field.getTag());
						}
